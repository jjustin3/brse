...
[_myDropPlane] spawn {
	...
	_PlayersDropCount = 30;
	switch (BRSEAICount) do {
		case 0: { _PlayersDropCount = 30; };
		case 1: { _PlayersDropCount = 30; };
		case 2: { _PlayersDropCount = 25; };
		case 3: { _PlayersDropCount = 25; };
		case 4: { _PlayersDropCount = 15; };
		case 5: { _PlayersDropCount = 15; };
	};

	_realHumaninDropPlane1 = count (crew _myDropPlane);
	dropCount = dropCount + 1;

	if (_realHumaninDropPlane1 > 20 && dropCount > 3) then { randomFloorpl1 = 40; };
	if (_realHumaninDropPlane1 > 20 && dropCount > 7) then { randomFloorpl1 = 5; };
	if (_realHumaninDropPlane1 < 11 && dropCount > 4) then {
		randomFloorpl1 = 30;
		_PlayersDropCount = 50;
	};
	if (_dist < 3400) then { randomFloorpl1 = 0; };
