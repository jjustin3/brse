MySpawnAI = {
	...
	if (_myplane == dropPlane2) then {
		_myplanePos set [2, _planePosZ + 300];
		_myplanePos set [1,_planePosY - 50];
	};
...
CheckButton = {
	...
	if (_button == 0) then { closeDialog 0; };
};
...
if (isFatigueEnabled < 1) then { player enableFatigue false; };

switch (BRSEAICount) do {
    case 0: {
		call getLastPointId;
		0 = [] spawn {
			player moveInCargo [dropPlane1, 15];
			player assignAsCargoIndex [dropPlane1, 15];
			dropPlane1 call MySpawnAI;
			sleep 20;
			dropPlane2 call MySpawnAI;
			sleep 20;
			dropPlane3 call MySpawnAI;
			playersCount = (sideEnemy countSide allUnits);
			sleep 1;
			call EventHandlerAdder;
			if (ExtTimeData > 0) then { call ExtendTimeInTriggers; };
			sleep 0.1;
			call EndTrGenerator;
		};
		call userAction1Handler;
	};
    case 1: {
		call getLastPointId;
		0 = [] spawn {
	...
	case 4: {
		call getLastPointId;
		ExpectedPlayerCount = 15;
		0 = [] spawn {
			player moveInCargo [dropPlane1,15];
			player assignAsCargoIndex [dropPlane1, 15];
			dropPlane1 call MySpawnAI;
			playersCount = (sideEnemy countSide allUnits);
			sleep 1; call EventHandlerAdder;
			if (ExtTimeData > 0) then { call ExtendTimeInTriggers; };
			sleep 0.1;
			call EndTrGenerator;
		};
...
switch (BRSEWeather) do {
    case 0: {};
    case 1: {
		_randFog = random [0.4,0.5,1];
		99999 setFog _randFog;
		forceWeatherChange;
	};
	case 2: {
		10 setOvercast 0.9;
