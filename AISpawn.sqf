...
_greenLight setSpeedMode "LIMITED";
};
MyPlayersCount = (sideEnemy countSide allUnits);
_randomRoute = 0;
genNumb = randompos;
runNumb = 0;
...
_wp setWaypointStatements ["true", "call ChangeLastPos"];
...
planeSpawnPositionArray = [[1589,2090,1250],[4371,1713,1250],[5950,1468,1250],[7411,1901,1250],[9785,3604,1250],[9693,6714,1250],[9133,7702,1250],[9012,8397,1250],[8797,9457,1250],[8907,10245,1250],[3885,10040,1250],[2200,8224,1250],[1829,7464,1250],[1664,6352,1250],[2200,3635,1250]];
...
switch (BRSEAICount) do {
    case 0: {call getLastPointId; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI;  sleep 20; dropPlane2 call MySpawnAI; sleep 20; dropPlane3 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
    case 1: {call getLastPointId; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI; sleep 20; dropPlane2 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
    case 2: {call getLastPointId; ExpectedPlayerCount = 10; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI; sleep 20; dropPlane2 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
	case 3: {call getLastPointId; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
	case 4: {call getLastPointId; ExpectedPlayerCount = 15; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
	case 5: {call getLastPointId; ExpectedPlayerCount = 30; 0 = [] spawn {player moveInCargo [dropPlane1,15]; player assignAsCargoIndex [dropPlane1, 15]; dropPlane1 call MySpawnAI; playersCount = (sideEnemy countSide allUnits); sleep 1; call EventHandlerAdder; if (ExtTimeData > 0) then {call ExtendTimeInTriggers;}; sleep 0.1; call EndTrGenerator;};  call userAction1Handler;};
