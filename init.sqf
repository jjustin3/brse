...
combatBehav = "COMBAT";
MainHeroesArray = ["C_man_1","C_man_polo_1_F_afro","C_man_polo_1_F_euro","C_man_polo_1_F_asia","C_man_1","C_man_polo_3_F_afro","C_man_polo_4_F_euro","C_man_polo_4_F_asia","C_man_polo_6_F","C_man_polo_6_F_afro","C_man_polo_6_F_euro","C_man_polo_6_F_asia","C_scientist_F","C_man_hunter_1_F","C_journalist_F","C_Orestes","C_Driver_2_F","C_Marshal_F","C_man_sport_1_F_euro","C_man_sport_1_F_afro","C_man_sport_1_F_asia","C_man_sport_2_F_euro","C_man_sport_2_F_afro","C_man_sport_2_F_asia","C_man_sport_3_F_euro","C_man_sport_3_F_afro","C_man_sport_3_F_asia","O_G_Survivor_F","B_Survivor_F","I_G_Survivor_F","CUP_C_C_Worker_01","CUP_C_C_Woodlander_02","CUP_C_C_Policeman_01","CUP_C_C_Profiteer_01","CUP_C_C_Villager_02","CUP_C_C_Villager_03","CUP_C_C_Fireman_01","CUP_C_C_Citizen_01","CUP_C_TK_Man_01_Waist","CUP_C_TK_Man_04","CUP_C_TK_Man_03_Waist","rhsusf_usmc_marpat_d_officer","rhs_vdv_driver","rhs_pilot","CUP_O_TK_Soldier_MG","CUP_I_PMC_Contractor2","CUP_I_PMC_Soldier_TL","CUP_I_PMC_Winter_Soldier_MG_PKM","CUP_I_TK_GUE_Commander","CUP_I_GUE_Soldier_AA","CUP_I_GUE_Soldier_GL","CUP_I_TK_GUE_Guerilla_Medic","CUP_I_TK_GUE_Soldier_GL","CUP_I_TK_GUE_Sniper","CUP_I_TK_GUE_Soldier_M16A2","CUP_I_TK_GUE_Soldier_HAT","CUP_I_TK_GUE_Soldier","CUP_I_PMC_Medic","I_C_Soldier_Bandit_7_F","I_C_Soldier_Bandit_6_F"];
BRSETime = 0;
isFatigueEnabled = 1;
...
BanPositionChange = false;

DropPlayersAverageC = 0;
DropPlayersX = 0;
DropPlayersY = 0;
...
SummXandY = {
	_AIToCalc = _this;
	_AIPOSCalc = getPosWorld _AIToCalc;
	_xCalc = _AIPOSCalc select 0;
	_yCalc = _AIPOSCalc select 1;
	DropPlayersX = DropPlayersX + _xCalc;
	DropPlayersY = DropPlayersY + _yCalc;
	DropPlayersAverageC = DropPlayersAverageC + 1;
};

ChangeLastPos = {
	if ((SafeZoneAIBased > 0) && !(BanPositionChange)) then {
		PlaneACK = PlaneACK + 1;
		switch (BRSEAICount) do {
			...
			case 3;
			case 4;
			case 5; { posBound = 1 }; // TODO* does this need to be part of case: 5?
		};
	
		if (PlaneACK > posBound) then {
		 _AverageX =  DropPlayersX / DropPlayersAverageC; // count
		 _AverageY= DropPlayersY / DropPlayersAverageC;
		 LastPosition = [_AverageX,_AverageY,1000];
		 LastPosition = [[[LastPosition, 500]],["water"]] call BIS_fnc_randomPos;
		};
 	};
};
	...
	{   
		...
		if (count _lootPlaces > 0) then {
			if ((_resultPump != 0) && (_resultKiosk != 0) && (_resultTransformer != 0) && (_resultPier != 0)) then {  
				{
					_amishouldSpawn = floor random (5);
...
// TODO* clean this up, maybe move to constants
...
lootTaker = {
	...
	{ [_hum, _x] call lootDeleterWp; } foreach _humobj; 
};
...
CamperKiller = {
	...
	{  
		...
		else { [_hum,_x] call waypointAdder; };
	} foreach _humnobj; 
};
	...
	0 = [_TotalSec] spawn { 
		...
		while { _TotalSec > 0 } do { 
			...
			if (_TotalSec >= 60) then {	
				...
				if (_checkDif < 10) then {
					if (_checkDif > 0) then {
						sleep _checkDif;
						_actSec = _actSec - _checkDif;
					};

					_TotalSec = _TotalSec - _checkDif;
					PutTimerSleep = 10;
					_TotalSec = _TotalSec - 10;
				} else {
					if (_actSec != 0) then {
						_TotalSec = _TotalSec - _actSec;
						PutTimerSleep = _actSec;
					} else { 
			...
			if (_infohint isEqualTo "none") then {}
			else { 1 cutText [_infohint, "PLAIN DOWN", 0.7]; }; 
			
			if (PutTimerSleep >0) then { sleep PutTimerSleep }; 
		};
		
		if (roundCounter < 10) then {
			_infohint = format ["THE PLAY AREA HAS BEEN RESTRICTED"];
			1 cutText [_infohint, "PLAIN DOWN", 0.7]; 
		} else {
			_infohint = format ["THE PLAY AREA IS STABLE NOW. KILL THEM ALL"];
			1 cutText [_infohint, "PLAIN DOWN", 0.7];
		};
	};
}; 
...
triggerLogicCreator = {
	0 = [] spawn {
		...
		if (roundCounter < 10) then { [_SmallCircleS, _BigCircleS] call graphicsCreator; }
		else { [_SmallCircleS, _BigCircleS] call graphicsCreatorEnd; };
		...
		if (roundCounter < 4 && enableDangerZones > 0) then { call DangerZoneGenerator;	};
		...
		if (roundCounter < 10) then { [_timeNote] call notifyGenerator; };

		{
			...
			if ((_x != player) && ((side _x) == sideEnemy)) then {
				...
				if (_dist < _SmallCircleS) then {
					...
					if ((_carWP > 0) && (count _cargAr > 0) && (roundCounter < 4)) then {
						_wp = _grp addWaypoint [position _myCar, 0];
						_grp setBehaviour 'CARELESS';
						_wp setWaypointType "GETIN NEAREST";
						_wp setWaypointStatements ["true", "_grpt = group this; _carGroupPos = [[[LastPosition, SmallCircleSGlobal]], ['water']] call BIS_fnc_randomPos; _a1 = _grpt addWaypoint [_carGroupPos, 0]; _a1 setWaypointType 'GETOUT'; _a1 setWaypointStatements['true', 'unassignVehicle this; moveOut this; _gr = group this; _gr setBehaviour combatBehav;'];"];
					} else {
						if (roundCounter < 6) then { [_x,_SmallCircleS] call runner; }
						else { [_x,_SmallCircleS] call lastPointRunner; };
					}; 
				};
			};
		} forEach allUnits;
			...
			0 = [] spawn {
				...
				
				{
					if ((_x != player) && ((side _x) == sideEnemy)) then {	
						...
						if (_distToCenter > SmallCircleSGlobal) then {
